...
Sometimes a dictionary built with
[`as_data_dictionary()`](https://perisphere-rwe.github.io/perinary/reference/as_data_dictionary.md)
includes more variables than you need for a particular dataset or
output.
[`select_variables()`](https://perisphere-rwe.github.io/perinary/reference/select_variables.md)
lets you keep, drop, or reorder variables using the same
[tidyselect](https://tidyselect.r-lib.org/) syntax as
...
share the same label. Because
[`index_rows()`](https://perisphere-rwe.github.io/perinary/reference/index_rows.md)
(described below) can match on either a variable’s name or its label,
two variables with the same label are ambiguous —
[`index_rows()`](https://perisphere-rwe.github.io/perinary/reference/index_rows.md)
will warn you and leave any row matching that label untouched. Dropping
one of the colliding variables with
[`select_variables()`](https://perisphere-rwe.github.io/perinary/reference/select_variables.md)
...
![A formatted regression table showing estimated body mass differences
(grams, 95% CI, and p-value) for penguin characteristics including
species, sex, and bill
dimensions.](reference/figures/README-regression-table-1.png)
...
[`index_rows()`](https://perisphere-rwe.github.io/perinary/reference/index_rows.md)
for model output specifically. You can use
[`index_rows()`](https://perisphere-rwe.github.io/perinary/reference/index_rows.md)
directly on any table that has columns identifying a variable name and
(for nominal variables) a category value — for instance, a summary table
you built by hand. The name/category columns can hold the dictionary’s
raw names and levels, their labels, or a mix of both, so this works even
on tables that have already been translated for a report:
...
[`set_variable_order()`](https://perisphere-rwe.github.io/perinary/reference/set_variable_order.md)),
`"Chinstrap"` sorts first within `species` (matching
[`set_category_order()`](https://perisphere-rwe.github.io/perinary/reference/set_labels.md)),
and the `sex` rows are correctly ordered even though they’re identified
by label (`"Male penguins"`/`"Female penguins"`) rather than the raw
level (`"male"`/`"female"`). Any row whose name or level doesn’t match
the dictionary is left untouched at its original position rather than
being reordered.
