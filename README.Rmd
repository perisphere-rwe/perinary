...
Second, we use `index_terms` to re-order the rows of these data based on the order they appear in the dictionary. This is helpful when you want variables to be listed in consistent order throughout your report. This is also where our earlier use of `set_variable_order()` has relevance - note how the variable ordering we set there is reflected in this table (i.e., species is first).
